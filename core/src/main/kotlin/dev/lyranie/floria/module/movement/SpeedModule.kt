...
class SpeedModule : ClientModule("speed", ClientCategory.MOVEMENT) {
    ...
    override fun onEvent(event: ClientEvent) {
        ...
        val speed = velocitySetting.value
        val movement = MovementHelper.getDirectionalVelocity(event.player, speed)

        event.player.setVelocity(movement.x, event.player.velocity.y, movement.z)
    }
}
